// PC1_AUGMENTED_LAGRANGE_IKF_RK4_COMPLETE.cpp
// COMPLETE: Augmented Lagrange + IKF REAL-TIME CORRECTION
// MATCHES: PC2_Augmented_Lagrange_Clean_NoPlots.m
...
int main() {
    ...
    const double dt_sub = dt_tcp ;  
    
    // ========== HYBRID BLENDING WEIGHT ==========
    const double alpha = 0.3;  // 30% IKF prediction, 70% AL (tunable!)
    
    std::cout << "=========================================================\n";
    std::cout << "C++: AUGMENTED LAGRANGE + IKF REAL-TIME CORRECTION\n";
    std::cout << "=========================================================\n";
    ...
    // Before any integration, record initial values from all sources
    {
        double t = 0.0;
        
        // Initial MATLAB values (same as AL initial)
        double theta_meas_init = al_state.q[2];
        double omega_meas_init = al_state.qdot[2];
        
        // Initial hybrid value = initial AL (alpha blending at t=0)
    ...
    while (true) {
        ...
        // ===== IKF PREDICTION (use for REAL-TIME CORRECTION) =====
        ikfPredict(&ikf_state, dt_tcp, m_A, g, L, I_theta);
        
        // ===== HYBRID CORRECTION: Blend AL + IKF for smooth, lag-free tracking =====
        ...
        // Update IKF with actual measurement (for next prediction accuracy)
