...
alpha_log = zeros(1, N); % Angular acceleration
...
for k = 1:N-1
    ...
    % Torque = r × F, where r = pivot-to-COM vector = [-L/2*sinθ, -L/2*cosθ]
    rx = -(L/2)*sin(q(3)); % x-component from pivot to COM
    ry = -(L/2)*cos(q(3)); % y-component from pivot to COM
    torque = rx*lambda(2) - ry*lambda(1); % τ = rx*Fy - ry*Fx (2D cross product)
    
    alpha = qddot(3); % Angular acceleration
    ...
    % ===============================================
    % STEP 8: TCP TRANSMISSION (θ, ω, α, τ)
    % ===============================================
    ...
    fwrite(tcpipClient, alpha_current, 'double'); % Angular acc [rad/s²]
    fwrite(tcpipClient, torque_current, 'double'); % Torque [N·m]
    
    if mod(k, 1000) == 0 % Progress every 1s
        fprintf('Step %d: θ=%.3f, ω=%.3f, α=%.3f, τ=%.3f\n', ...
...
fprintf(' Transmitted: θ, ω, α, τ to C++ server\n');
fprintf('===============================================\n\n');
...
function dydt = augmentedLagrangeODE(y, m_A, g, L, I_theta)
    ...
    Cqq_qdot2 = [qdot(3)^2*(L/2)*cos(q(3)); 
    -qdot(3)^2*(L/2)*sin(q(3))];
    A = [M, Cq'; Cq, zeros(2)]; b = [Qe + Qv; -Cqq_qdot2];
    sol = A\b; qddot = sol(1:3);
    dydt = [qdot; qddot];
end
